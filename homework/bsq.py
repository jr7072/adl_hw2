...
class BSQ(torch.nn.Module):
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        codebook_logits: torch.Tensor = self.down_projection_layer(x)
        codebook_logit_l2_norm: torch.Tensor = (codebook_logits ** 2).sum(dim=1).sqrt()[:, None]
        normalized_codebook_logits = codebook_logits / codebook_logit_l2_norm
        binary_vector = diff_sign(normalized_codebook_logits)
    ...
    def encode_index(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = chw_to_hwc(x)
        return self._code_to_index(y)

    def decode_index(self, x: torch.Tensor) -> torch.Tensor:
        """
        Decode a set of integer tokens into an image.
        """
        y = self._index_to_code(x)
        return hwc_to_chw(y)
    ...
    def _index_to_code(self, x: torch.Tensor) -> torch.Tensor:
        return 2 * ((x[..., None] & (2 ** torch.arange(self._codebook_bits).to(x.device))) > 0).float() - 1 # ty: ignore
...
class BSQPatchAutoEncoder(PatchAutoEncoder, Tokenizer):
    ...
    def encode_index(self, x: torch.Tensor) -> torch.Tensor:
        y = self.encode(x)
        return self.quantizer.encode_index(y)

    def decode_index(self, x: torch.Tensor) -> torch.Tensor:
        y = self.quantizer.decode_index(x)
        return self.decode(y)

    def encode(self, x: torch.Tensor) -> torch.Tensor:
        
        encoded_image = super().encode(x)
        return self.quantizer.encode(encoded_image)

    def decode(self, x: torch.Tensor) -> torch.Tensor:
        
        dequantized_image = self.quantizer.decode(x)
        return super().decode(dequantized_image)
